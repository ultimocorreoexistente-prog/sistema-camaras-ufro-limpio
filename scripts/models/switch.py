...
port_speed = Column(String(0), nullable=True,
...
used_ports = len([p for p in self.puertos if not p.is_available()])
return round((used_ports / self.total_ports) * 100, )
...
speed = Column(String(0), nullable=True,
...
return cls.query.filter_by(
switch_id=switch_id,
connected_equipment_id=isnot(None),
deleted=False
).all()
...
comment="Subred asignada (ej: 19.168.1.0/4)")
