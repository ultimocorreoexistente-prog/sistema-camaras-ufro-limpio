...
equipo = EQUIPOS_MAP[m.tipo_equipo].query.filter_by(id=m.equipo_id).first()
if equipo:
equipo_info = {
'id': equipo.id,
...
'tiempo_estimado_horas': getattr(m, 'tiempo_estimado_horas', None),
'costo_estimado': getattr(m, 'costo_estimado', None),
'costo_real': getattr(m, 'costo_real', None),
...
historial = getattr(mantenimiento, 'historial', [])
...
'prioridad': getattr(mantenimiento, 'prioridad', 'media'),
'fecha_programada': mantenimiento.fecha_programada.isoformat(),
'fecha_ejecucion': mantenimiento.fecha_ejecucion.isoformat() if mantenimiento.fecha_ejecucion else None,
'tiempo_estimado_horas': getattr(mantenimiento, 'tiempo_estimado_horas', None),
'tiempo_real_horas': getattr(mantenimiento, 'tiempo_real_horas', None),
'costo_estimado': getattr(mantenimiento, 'costo_estimado', None),
'costo_real': getattr(mantenimiento, 'costo_real', None),
'notas': getattr(mantenimiento, 'notas', ''),
...
mantenimiento.prioridad = data['prioridad']
...
if data[campo] == 'completado':
mantenimiento.fecha_ejecucion = datetime.utcnow()
mantenimiento.ejecutado_por_id = current_user.id
elif campo == 'fecha_programada':
mantenimiento.fecha_programada = datetime.fromisoformat(data[campo].replace('Z', '+00:00')) if data[campo] else None
...
func.count(Mantenimiento.id)
).group_by(Mantenimiento.tipo_equipo).all()
...
if tiempos_real:
tiempo_promedio_real = round(sum(tiempos_real) / len(tiempos_real), )
if tiempos_estimados:
tiempo_promedio_estimado = round(sum(tiempos_estimados) / len(tiempos_estimados), )
