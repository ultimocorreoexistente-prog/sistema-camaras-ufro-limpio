...
# 🟢 CORRECCIÓN 1: Solamente importar 'text'. 'metadata' no se importa aquí para evitar el error.
...
        if pd.isna(value):
            return None
        # Intentar convertir a int directamente si es str/float y tiene solo números
...
            # Intenta parsear varios formatos de fecha comunes
...
                except ValueError:
                    continue
            # Si es un número (días desde 1899), intenta convertir desde Excel
            if isinstance(value, (int, float)) and value > 0:
                # Asumimos que es un formato de fecha serial de Excel
                return pd.to_datetime(value, unit='D', origin='1899-12-30').date()
                
        return value.date() if hasattr(value, 'date') else None
    except Exception as e:
        # print(f"Error al convertir fecha: {value} - {e}")
        return None
...
    # Inicializar con una zona genérica, asumiendo que el campo Campus o Edificio de Ubicacion se usará
    zona_actual = 'Desconocida'
    
    for linea in lineas:
        # Detectar zona/ubicación (primera columna de la tabla)
        if '|' in linea and not linea.strip().startswith('|---|'): # Ignorar líneas de separador de tabla
            partes = [p.strip() for p in linea.split('|') if p.strip()] # Limpiar y filtrar partes
            
            if len(partes) >= 5: # Asegurar que haya suficientes columnas para Ubicación (col 1) y Observación (col 5)
                zona_candidata = partes[0].strip()
                # Si la zona candidata no es un encabezado o está vacía, actualiza la zona
                if zona_candidata and zona_candidata.lower() not in ('ubicacion', 'zona'):
                    zona_actual = zona_candidata
                
                # La observación es el quinto campo (índice 4 si contamos desde 0)
...
                            # El grupo 1 es el nombre o código de la cámara
...
        count = 0
        for _, row in df.iterrows():
            # 🟢 CORRECCIÓN 1: Usar EquipoTecnico
...
        count = 0
        # 🟢 Asumo que el modelo se llama CatalogoTipoFalla
        for _, row in df.iterrows():
...
        count = 0
        # 🟢 Asumo que el modelo se llama UPS
        for _, row in df.iterrows():
            ups = UPS(
                codigo=safe_str(row.get('Codigo')),
                modelo=safe_str(row.get('Modelo')),
...
        count = 0
        # 🟢 Asumo que el modelo se llama NVR
        for _, row in df.iterrows():
            nvr = NVR(
                codigo=safe_str(row.get('Codigo')),
...
        count = 0
        # 🟢 Asumo que el modelo se llama FuentePoder
        for _, row in df.iterrows():
            fuente = FuentePoder(
                codigo=safe_str(row.get('Codigo')),
                modelo=safe_str(row.get('Modelo')),
...
        # 🟢 Crear usuario admin si no existe (esto debería ser manejado por ejecutar_migracion)
        # Se deja el código para asegurar la disponibilidad si se llama a migrar_datos() directamente
        if not admin_user:
...
                # 🟢 NOTA: Estos IDs deben coincidir con los insertados en el paso 3.
                tipo_falla_map = {
                    'Telas de araña': 1,
                    'Imagen borrosa': 1,
                    'Mica rallada': 2,
...
        
        # ❌ LÍNEA ORIGINAL (Causaba el error DependentObjectsStillExist):
        # db.drop_all() 

        # 🟢 CORRECCIÓN 2: Usar DROP CASCADE y db.metadata.drop_all para eliminar dependencias de forma forzada
        try:
            # 🟢 Al usar db.metadata, ya no necesitamos la importación explícita de 'metadata' que fallaba.
            with db.engine.begin() as connection:
                # Tablas dependientes (deben eliminarse primero o con CASCADE)
                connection.execute(text("DROP TABLE IF EXISTS fotografia CASCADE"))
                connection.execute(text("DROP TABLE IF EXISTS mantenimiento CASCADE"))
                connection.execute(text("DROP TABLE IF EXISTS puerto_switch CASCADE"))
                # Otras tablas con muchas dependencias
                connection.execute(text("DROP TABLE IF EXISTS camara CASCADE"))
                connection.execute(text("DROP TABLE IF EXISTS falla CASCADE"))
                connection.execute(text("DROP TABLE IF EXISTS usuario CASCADE"))
                connection.execute(text("DROP TABLE IF EXISTS equipo_tecnico CASCADE"))
                
                # Usamos db.metadata.drop_all para limpiar el resto de las tablas y forzar la eliminación del esquema
                db.metadata.drop_all(connection, checkfirst=True)
            
        except Exception as e:
            print(f"❌ Error durante la limpieza de la base de datos (DROP CASCADE): {e}")
            raise
        
        # 2. Creación del Esquema
        try:
            db.create_all()
            print("✓ Base de datos limpia y esquema creado.")
        except Exception as e:
